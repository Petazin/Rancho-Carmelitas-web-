...
- **Número de Habitaciones Parametrizable en Cabañas:** Se implementó el campo de habitaciones (`bedrooms`) en el administrador de cabañas (`/admin/cabanas`), permitiendo crearlas y editarlas con su valor real en Supabase (tabla `cabins`). En el detalle de la cabaña (`/cabins/[id]`), se consume dinámicamente este campo; si está vacío en base de datos, aplica un fallback redondeando la capacidad al entero inferior (`Math.floor(capacidad / 2)`) y despliega un badge amarillo animado Stitch UI de advertencia en el listado del administrador para incentivar su configuración.
- **Script SQL de Habitaciones:** Creación del script de migración `schema_update_cabins_bedrooms.sql` en el directorio raíz para inyectar la columna en Supabase.
- **Hotfix de Diseño Adaptable en Hero de Portada:** Se resolvió el desbordo de contenido y el recorte de los botones `Ver Disponibilidad` y `Ver Instalaciones` en resoluciones bajas o al aumentar el zoom de pantalla. Se reemplazó la altura rígida del Hero `h-[80vh] min-h-[600px]` y su recorte `overflow-hidden` por una altura mínima adaptativa `min-h-[85vh]` y relleno vertical `py-20 sm:py-24 md:py-28`, permitiendo que el contenedor del banner crezca de manera fluida y adaptativa según la extensión de la descripción.
- **Corrección de Mapeo de Habitaciones en Catálogo Público:** Se actualizó la renderización de `CabinCard` en el listado del Home para usar la propiedad de base de datos `cabin.bedrooms` y aplicar el fallback `Math.floor` en lugar de una fórmula de redondeo hacia arriba estática que creaba discrepancias con el detalle.
...
- **Corrección de Token de Service Role en Enlace de Recuperación:** Se identificó que al reenviar la recuperación de contraseña para un usuario ya confirmado (`isConfirmed`), el backend invocaba `resetPasswordForEmail` utilizando el cliente administrativo `supabaseAdminLocal` (con clave `SUPABASE_SERVICE_ROLE_KEY`). Esto causaba que Supabase emitiera erróneamente un token maestro con rol `service_role` en la URL de redirección en lugar de un token de usuario (`authenticated`), lo que hacía que `updateUser` en el frontend fallara con "Auth session missing!". Se solucionó inicializando un cliente Supabase público temporal (`supabaseAnon` con la anon key pública) para disparar el correo de recuperación en el backend de forma segura y correcta.
